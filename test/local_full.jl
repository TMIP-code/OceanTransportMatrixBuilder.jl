@testitem "velocities and mass transports" setup = [LocalBuiltMatrix] tags = [:skipci, :local_full] begin
    ...
    (;
        ...
        umo, vmo, umo_lon, umo_lat, vmo_lon, vmo_lat, model, member, outputdir,
    ) = LocalBuiltMatrix
    ...
    arrows2d!(ax, uo2_lon[i, j, k] |> vec, uo2_lat[i, j, k] .+ 0.02 |> vec, umo[i, j, k] |> vec, 0(umo[i, j, k] |> vec), lengthscale = 3.0e-8, tipcolor = (:blue, 0.3), shaftcolor = (:blue, 0.3))
    arrows2d!(ax, vo2_lon[i, j, k] .+ 0.02 |> vec, vo2_lat[i, j, k] |> vec, 0(vmo[i, j, k] |> vec), vmo[i, j, k] |> vec, lengthscale = 3.0e-8, tipcolor = (:red, 0.3), shaftcolor = (:red, 0.3))
    fig
    ...
    arrows2d!(ax, uo2_lon[i, j, k] |> vec, uo2_lat[i, j, k] |> vec, uo2[i, j, k] |> vec, 0(uo2[i, j, k] |> vec), lengthscale = 10, tipcolor = :blue, shaftcolor = :blue)
    arrows2d!(ax, vo2_lon[i, j, k] |> vec, vo2_lat[i, j, k] |> vec, 0(vo2[i, j, k] |> vec), vo2[i, j, k] |> vec, lengthscale = 10, tipcolor = :red, shaftcolor = :red)
    arrows2d!(ax, uo2_lon[i, j, k] |> vec, uo2_lat[i, j, k] .+ 0.02 |> vec, umo[i, j, k] |> vec, 0(umo[i, j, k] |> vec), lengthscale = 1.0e-8, tipcolor = (:blue, 0.3), shaftcolor = (:blue, 0.3))
    arrows2d!(ax, vo2_lon[i, j, k] .+ 0.02 |> vec, vo2_lat[i, j, k] |> vec, 0(vmo[i, j, k] |> vec), vmo[i, j, k] |> vec, lengthscale = 1.0e-8, tipcolor = (:red, 0.3), shaftcolor = (:red, 0.3))
    fig
